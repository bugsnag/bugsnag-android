...
ssize_t bsg_unwind_crash_stack(bugsnag_stackframe stack[BUGSNAG_FRAMES_MAX],
  ...
  for (auto &frame : local_unwinder->frames()) {
    ...
    if (frame.map_exact_offset < frame.map_start) {
      // If the offset is less than the start, we need to adjust the load
      // address to account for the offset.
...
bsg_unwind_concurrent_stack(bugsnag_stackframe stack[BUGSNAG_FRAMES_MAX],
  ...
  for (auto &frame : frames) {
    ...
    if (frame.map_info != nullptr) {
      ...
      if (map_info->offset() < map_info->start()) {
        // If the offset is less than the start, we need to adjust the load
        // address to account for the offset.
        dst_frame.load_address = map_info->start() - map_info->offset();
      } else {
        dst_frame.load_address = map_info->start();
      }
