...
void bsg_serialize_device(const bsg_device_info device, JSON_Object *event_obj) {
  ...
  if (device.time > 0) {
    strftime(event_time, sizeof event_time, "%FT%TZ", gmtime(&device.time));
    json_object_dotset_string(event_obj, "device.time", event_time);
  }
}
...
char *bsg_serialize_event_to_json_string(bugsnag_event *event) {
  ...
  char *serialized_string = NULL;
  {
    ...
    bsg_serialize_exception(event->error, exception, stacktrace);
