...
JNIEXPORT void JNICALL Java_com_bugsnag_android_ndk_NativeBridge_install(
  ...
  bugsnag_event_set_app_version(&event, app_version);
  bugsnag_event_set_build_uuid(&event, build_uuid);
  bugsnag_event_set_release_stage(&event, release_stage);
...
Java_com_bugsnag_android_ndk_NativeBridge_clearBreadcrumbs(JNIEnv *env,
  ...
  if (bsg_global_env == NULL)
    return;
  bsg_request_env_write_lock();
  bugsnag_event_clear_breadcrumbs(&bsg_global_env->next_event);
  bsg_release_env_write_lock();
}
