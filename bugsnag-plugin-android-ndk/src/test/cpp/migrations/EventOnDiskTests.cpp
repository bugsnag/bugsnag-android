...
/**
 * Create a new event in the current format
 */
...
Java_com_bugsnag_android_ndk_migrations_EventOnDiskTests_generatePayloadInfo(
    JNIEnv *env, jobject _this, jstring temp_file) {
  const char *path = write_event(env, temp_file, create_payload_info_event);
...
Java_com_bugsnag_android_ndk_migrations_EventOnDiskTests_generateAndStoreEvent(
    JNIEnv *env, jobject _this, jstring temp_file) {
  const char *path = write_event(env, temp_file, create_full_event);
