...
class AppDataCollector {
    ...
    AppData generateAppData() {
        ...
        data.setDurationInForeground(calculateDurationInForeground());
    ...
    void addAppMetaData(MetaData metaData) {
        ...
        metaData.addToTab("app", "lowMemory", isLowMemory());
    }

    private void populateAppDataSummary(AppDataSummary data) {
        Configuration config = client.config;
        data.setVersionCode(calculateVersionCode());
        data.setCodeBundleId(config.getCodeBundleId());
        data.setNotifierType(calculateNotifierType(config));
        data.setReleaseStage(guessReleaseStage());
        data.setVersionName(calculateVersionName());
    }
