...
class DeviceDataCollector {
    ...
    DeviceDataCollector(Client client) {
        this.client = client;
    ...
    DeviceDataSummary generateDeviceDataSummary() {
        DeviceDataSummary data = new DeviceDataSummary();
        populateDeviceDataSummary(data);
        return data;
    }
    ...
    DeviceData generateDeviceData() {
        ...
        populateDeviceDataSummary(data);
        data.setId(id);
        data.setTotalMemory(calculateTotalMemory());
        data.setFreeMemory(calculateFreeMemory());
        data.setFreeDisk(calculateFreeDisk());
        data.setOrientation(calculateOrientation());
        return data;
    }

    private void populateDeviceDataSummary(DeviceDataSummary data) {
        data.setManufacturer(Build.MANUFACTURER);
        data.setModel(Build.MODEL);
        data.setOsName("android");
        data.setOsVersion(Build.VERSION.RELEASE);
        data.setJailbroken(isRooted());
    }

    void populateDeviceMetaData(MetaData metaData) {
        metaData.addToTab("device", "batteryLevel", getBatteryLevel());
        metaData.addToTab("device", "charging", isCharging());
        metaData.addToTab("device", "locationStatus", getLocationStatus());
        metaData.addToTab("device", "networkAccess", getNetworkAccess());
        ...
        metaData.addToTab("device", "emulator", emulator);
