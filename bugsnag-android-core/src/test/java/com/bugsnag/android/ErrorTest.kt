...
class ErrorTest {
    ...
    @Test
    fun createError() {
        val err = Error.createError(RuntimeException("Whoops"), true, setOf(), NoopLogger)
    ...
    fun createNestedError() {
        val err = Error.createError(
            IllegalStateException("Some err", RuntimeException("Whoops")),
            true,
            setOf(),
            NoopLogger
        )
