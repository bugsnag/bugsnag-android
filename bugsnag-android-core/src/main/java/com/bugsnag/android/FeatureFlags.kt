...
internal class FeatureFlags private constructor(
    ...
    /*
     * Implemented as *effectively* a CopyOnWriteArrayList - but since FeatureFlags are effective
     * key/value pairs CopyOnWriteArrayList requires significantly more complexity when replacing
     * existing FeatureFlags or even being certain that we're not adding duplicates. Specifically
     * locking outside of the CopyOnWriteArrayList so that we can 1) locate the existing entry and
     * then 2) remove or replace it without the list changing between the two calls.
     *
     ...
     *
     * It's assumed that *most* FeatureFlags will be added:
     * a) in bulk (addFeatureFlags)
     * b) up-front, or during the normal app lifecycle (not during an Event)
     *
     * As such as copy-on-write structure allows an Event to simply capture a reference to the
     * "snapshot" of FeatureFlags that were active when the Event was created, without any need
     * for deep copying, keeping the expense off the crash paths.
     */
