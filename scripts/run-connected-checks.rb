...
begin
  ...
  emulator_thread = Thread.new do
    PTY.spawn('emulator', '-avd', "test-sdk-#{target_api_level}", '-no-window', '-gpu', 'swiftshader', '-noaudio', '-no-boot-anim', '-camera-back', 'none', '-no-snapshot-load') do |stdout, _stdin, pid|
      emulator_pid = pid
      stdout.each do |line|
        emulator_lines << line
        puts line
