...
Scenario: When a session is resumed the error uses the previous session information
    ...
    Then I wait to receive 3 requests
    And the session is valid for the session reporting API version "1.0" for the "Android Bugsnag Notifier" notifier
    And the error payload field "sessions.0.id" is stored as the value "resumed_session_id"
    And I discard the oldest request
    And the error is valid for the error reporting API version "4.0" for the "Android Bugsnag Notifier" notifier
...
Scenario: When a new session is started the error uses different session information
    ...
    And the session is valid for the session reporting API version "1.0" for the "Android Bugsnag Notifier" notifier
    And the error payload field "sessions.0.id" is stored as the value "first_new_session_id"
    And I discard the oldest request
    And the error is valid for the error reporting API version "4.0" for the "Android Bugsnag Notifier" notifier
    And the error payload field "events.0.session.id" equals the stored value "first_new_session_id"
    And I discard the oldest request
    And the session is valid for the session reporting API version "1.0" for the "Android Bugsnag Notifier" notifier
    And the error payload field "sessions.0.id" is stored as the value "second_new_session_id"
    And I discard the oldest request
    And the error is valid for the error reporting API version "4.0" for the "Android Bugsnag Notifier" notifier
