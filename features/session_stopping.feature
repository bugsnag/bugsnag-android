...
Scenario: When a new session is started the error uses different session information
    ...
    And I discard the oldest request
    And the request is valid for the error reporting API version "4.0" for the "Android Bugsnag Notifier" notifier
# Not currently implemented  |  And the payload field "events.0.session.events.handled" equals 1 for request 1
# Not currently implemented  |  And the payload field "events.0.session.events.handled" equals 1 for request 3
# Not currently implemented  |   And the payload field "events.0.session.id" of request 1 does not equal the payload field "events.0.session.id" of request 3
